...
export function SiteFooter({ locale, m, interior = false }: { locale: Locale; m: Messages; interior?: boolean }) {
  const targets = interior ? navigationTargets.map((target) => `/${locale}/${target}`) : navigationTargets;
  return <footer className="site-footer">
    <div className="footer-nav"><nav className="shell" aria-label={m.footerServices}>{m.nav.map((name,i) => <a key={name} href={targets[i]}>{name}</a>)}<a href={interior ? `/${locale}/#consultation` : "#consultation"}>{m.consult}</a></nav></div>
